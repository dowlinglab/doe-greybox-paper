...
class MembraneExperiment(Experiment):
    def __init__(self, data, theta=None):
        ...
        if theta is None:
            self.theta = {
                "fs.Lp": 1e-7,
                "fs.constant_sieving_coeff[Li]": 1.0,
                "fs.constant_sieving_coeff[Co]": 0.4,
                "fs.ionic_strength_coeff[Li]": 8e-4,
                "fs.ionic_strength_coeff[Co]": 4e-4,
            }
        else:
            self.theta = theta
        self.model = None
    ...
    def create_model(self):
        ...
        m.fs.Lp = pyo.Var(
            units=pyo.units.m / (pyo.units.hour * pyo.units.Pa), bounds=(1e-8, None)
        )
        ...
        m.fs.constant_sieving_coeff = pyo.Var(
            m.fs.solutes, units=pyo.units.dimensionless, bounds=(1e-8, None)
        )
        m.fs.constant_sieving_coeff["Li"].fix(
            self.theta["fs.constant_sieving_coeff[Li]"]
        )
        m.fs.constant_sieving_coeff["Co"].fix(
            self.theta["fs.constant_sieving_coeff[Co]"]
        )

        # add the ionic strength coefficient of the species
        m.fs.ionic_strength_coeff = pyo.Var(
            m.fs.solutes, units=pyo.units.m**3 / pyo.units.mol, bounds=(1e-8, None)
        )
        ...
        m.fs.stage1.ionic_strength = pyo.Var(
            m.fs.stage1.elements, units=pyo.units.mol / pyo.units.m**3
        )
        ...
        def ionic_strength_rule(b, s):
            ...
            return b.ionic_strength[s] == (1 / 2) * sum(
                (in_state.conc_mass_solute[j] / m.fs.molar_mass[j])
                * m.fs.valency[j] ** 2
        ...
        m.fs.stage1.ionic_strength_eqtn = pyo.Constraint(
            m.fs.stage1.elements, rule=ionic_strength_rule
        )

        # adding species sieving coefficient
        m.fs.stage1.sieving_coefficient = pyo.Var(
        ...
        def sieving_coeff_rule(b, s, j):
            ...
            if j == "Li":
                return (
                    ...
                    == m.fs.constant_sieving_coeff[j]
                    + m.fs.ionic_strength_coeff[j] * b.ionic_strength[s]
                )
            else:
                return (
                    b.sieving_coefficient[s, j]
        ...
        m.fs.stage1.sieving_coeff_eqtn = pyo.Constraint(
            m.fs.stage1.elements, m.fs.solutes, rule=sieving_coeff_rule
        )
        ...
        def osmotic_pressure_rule(b, s):
            ...
            c_perm = sum(
                b.sieving_coefficient[s, j]
                * b.retentate[0, s].conc_mass_solute[j]
                / m.fs.molar_mass[j]
                for j in m.fs.solutes
            )

            return b.osmotic_pressure[s] == R * m.fs.temperature * (c_ret - c_perm)

        # add the osmotic pressure constraint
        m.fs.stage1.osmotic_pressure_eqtn = pyo.Constraint(
            m.fs.stage1.elements, rule=osmotic_pressure_rule
        )

        # add the water flux
        m.fs.stage1.water_flux = pyo.Var(
            m.fs.stage1.elements,
        ...
        def water_flux_rule(b, s):
            ...
            Returns:
                Water flux equation"""
            return b.water_flux[s] == m.fs.Lp * (
                m.fs.pressure_drop - b.osmotic_pressure[s]
            )

        # add the water flux constraint
        m.fs.stage1.water_flux_eqtn = pyo.Constraint(
            m.fs.stage1.elements, rule=water_flux_rule
        )
        ...
        m.fs.stage2.ionic_strength = pyo.Var(
            m.fs.stage2.elements, units=pyo.units.mol / pyo.units.m**3
        )
        m.fs.stage2.ionic_strength_eqtn = pyo.Constraint(
            m.fs.stage2.elements, rule=ionic_strength_rule
        )

        # add the sieving coefficient
        m.fs.stage2.sieving_coefficient = pyo.Var(
            m.fs.stage2.elements,
            m.fs.solutes,
            units=pyo.units.dimensionless,
            bounds=(1e-8, None),
        )
        m.fs.stage2.sieving_coeff_eqtn = pyo.Constraint(
            m.fs.stage2.elements, m.fs.solutes, rule=sieving_coeff_rule
        )

        # add the osmotic pressure
        m.fs.stage2.osmotic_pressure = pyo.Var(m.fs.stage2.elements, units=pyo.units.Pa)
        m.fs.stage2.osmotic_pressure_eqtn = pyo.Constraint(
            m.fs.stage2.elements, rule=osmotic_pressure_rule
        )

        # add the water flux
        m.fs.stage2.water_flux = pyo.Var(
            m.fs.stage2.elements,
            units=pyo.units.m / pyo.units.hour,
            bounds=(1e-8, None),
        )
        m.fs.stage2.water_flux_eqtn = pyo.Constraint(
            m.fs.stage2.elements, rule=water_flux_rule
        )
        ...
        # add the ionic strength
        m.fs.stage3.ionic_strength = pyo.Var(
            m.fs.stage3.elements, units=pyo.units.mol / pyo.units.m**3
        )
        ...
        def stage3_ionic_strength_rule(b, s):
            ...
            if s == 1:
                ...
                q_in = in_state.flow_vol + side_state.flow_vol
        ...
        # add the ionic strength constraint
        m.fs.stage3.ionic_strength_eqtn = pyo.Constraint(
            m.fs.stage3.elements, rule=stage3_ionic_strength_rule
        )

        # add the sieving coefficient
        m.fs.stage3.sieving_coefficient = pyo.Var(
            m.fs.stage3.elements,
            m.fs.solutes,
            units=pyo.units.dimensionless,
            bounds=(1e-8, None),
        )
        m.fs.stage3.sieving_coeff_eqtn = pyo.Constraint(
            m.fs.stage3.elements, m.fs.solutes, rule=sieving_coeff_rule
        )

        # add the osmotic pressure
        m.fs.stage3.osmotic_pressure = pyo.Var(m.fs.stage3.elements, units=pyo.units.Pa)
        m.fs.stage3.osmotic_pressure_eqtn = pyo.Constraint(
            m.fs.stage3.elements, rule=osmotic_pressure_rule
        )

        # add the water flux
        m.fs.stage3.water_flux = pyo.Var(
            m.fs.stage3.elements,
            units=pyo.units.m / pyo.units.hour,
            bounds=(1e-8, None),
        )
        m.fs.stage3.water_flux_eqtn = pyo.Constraint(
            m.fs.stage3.elements, rule=water_flux_rule
        )
        ...
        m.fs.stage3.retentate_side_stream_state[0, 10].flow_vol.fix(
    ...
    def finalize_model(self):
        ...
        m.fs.mix2_inlet_flow_LB_con = pyo.Constraint(
            expr=m.fs.mix2.inlet_1.flow_vol[0] >= 27
        )
        m.fs.mix2.inlet_1.flow_vol[0].setub(33)
    ...
    def label_experiment(self):
        ...
        m.experiment_inputs.update(
            [
                ...
                (
                    m.fs.stage3.retentate_side_stream_state[0, 10].conc_mass_solute[
                        "Li"
                    ],
                    self.data["C_Li_feed (kg/m^3)"],
                ),
        ...
        m.measurement_error.update(
            [(m.fs.stage1.retentate_outlet.conc_mass_solute[0, "Co"], conc_std_dev)]
        )
        # Add measurement error for Li_Co_prdt
        m.measurement_error.update(
            [(m.fs.stage1.retentate_outlet.conc_mass_solute[0, "Li"], conc_std_dev)]
        )
        # Add measurement error for Li_Li_prdt
        m.measurement_error.update(
            [(m.fs.stage3.permeate_outlet.conc_mass_solute[0, "Li"], conc_std_dev)]
        )
        # Add measurement error for Co_Li_prdt
        m.measurement_error.update(
            [(m.fs.stage3.permeate_outlet.conc_mass_solute[0, "Co"], conc_std_dev)]
        )
        # Add measurement error for Q_Co_prdt
        m.measurement_error.update(
            [(m.fs.stage1.retentate_outlet.flow_vol[0], flowrate_std_dev)]
        )
        # Add measurement error for Q_Li_prdt
        m.measurement_error.update(
            [(m.fs.stage3.permeate_outlet.flow_vol[0], flowrate_std_dev)]
        )
