...
class TC_Lab_experiment(Experiment):
    def __init__(
        ...
        if sine_amplitude is not None and sine_period is not None:
            ...
            assert (
                sine_period <= self.sine_period_max
            ), "Sine period must be less than " + str(self.sine_period_max)
            assert (
            ...
            raise ValueError(
                "If sine wave is used, both amplitude and period must be provided."
            )
        else:
    ...
    def create_model(self):
        ...
        if not self.reparam:
            ...
            m.inv_CpH = pyo.Var(
                initialize=self.theta_initial["inv_CpH"], bounds=(0, 1e6)
            )
            m.inv_CpH.fix()
            m.inv_CpS = pyo.Var(
                initialize=self.theta_initial["inv_CpS"], bounds=(0, 1e3)
            )
            m.inv_CpS.fix()
        else:
            # REPARAMETRIZATION
            m.beta_1 = pyo.Var(
                initialize=self.theta_initial["Ua"] * self.theta_initial["inv_CpH"],
                bounds=(0, 1e6),
            )
            m.beta_1.fix()
            m.beta_2 = pyo.Var(
                initialize=self.theta_initial["Ub"] * self.theta_initial["inv_CpH"],
                bounds=(1e-6, 1e6),
            )
            m.beta_2.fix()
            m.beta_3 = pyo.Var(
                initialize=self.theta_initial["Ub"] * self.theta_initial["inv_CpS"],
                bounds=(0, 1e6),
            )
            m.beta_3.fix()
            m.beta_4 = pyo.Var(
                initialize=self.alpha * pyo.value(m.P1) * self.theta_initial["inv_CpH"],
                bounds=(0, 1e6),
            )
            m.beta_4.fix()
    ...
    def finalize_model(self):
        ...
        if self.sine_amplitude is not None and self.sine_period is not None:
            ...
            m.u1_period = pyo.Var(
                initialize=self.sine_period,
                bounds=(self.sine_period_min, self.sine_period_max),
            )  # minutes
            m.u1_amplitude = pyo.Var(
                initialize=self.sine_amplitude, bounds=(0, 50)
            )  # % power
            ...
            def u1_constraint(m, t):
                return m.U1[t] == 50 + m.u1_amplitude * sin(
                    2 * np.pi / (m.u1_period * 60) * t
                )
    ...
    def label_experiment(self):
        ...
        if self.include_Th:
            m.experiment_outputs.update(
                (m.Th1[t], self.data.T1[ind]) for ind, t in enumerate(self.data.time)
            )
        if self.number_of_states == 4:
            m.experiment_outputs.update(
            ...
            if self.include_Th:
                m.experiment_outputs.update(
                    (m.Th2[t], self.data.T2[ind])
                    for ind, t in enumerate(self.data.time)
                )
        ...
        if self.number_of_states == 4:
            m.measurement_error.update(
                (m.Ts2[t], 0.25) for ind, t in enumerate(self.data.time)
            )
            if self.include_Th:
